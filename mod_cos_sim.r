...
for (i in 12:20) {
    cpltr[is.na(cpltr[,i]),i] <- 0
    cpltr[cpltr[,i]>1,i] <- 1
    cplte[is.na(cplte[,i]),i] <- 0
    cplte[cplte[,i]>1,i] <- 1
}
...
require(Matrix)
weightm <- c(1.962, 1.25, 1.25, 1.00, 6.50, 0.625, 0.4) # males weights
weightf <- c(1.75, 0.75, 1.50, 1.00, 6.50, 0.625, 0.25) # female weights
...
write.csv(submission, file="submission/mod_cos_sim_r.csv", row.names=FALSE)
